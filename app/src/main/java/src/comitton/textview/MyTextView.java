...
//			}
	...
	public void drawScreen(Canvas canvas) {
		...
		int cx;
		int cy;
		if (mEffectDraw && mBackBitmap != null) {
			// エフェクト中
			// 前のページ描画
...
//			Logcat.d(logLevel, "alpha" + mEffectRate);
		...
		synchronized (mLock) {
			mDrawBreak = false;
			for (int p = st; p <= ed; p++) {
				if (p < 0 || mTextData == null || p >= mTextData.length || mTextData[p] == null) {
					continue;
				}
...
//		Logcat.d(logLevel, "index=" + page);
	...
	private void TextDraw(Canvas canvas, int index, float x, float y) {
		...
		for (int i = 0; i < tdds.length && !mDrawBreak; i++) {
				...
				if (picIndex >= 0 && picIndex < mPictures.length) {
					...
					if (picdata != null && !picdata.mIsError) {
						...
						if (bmdraw != null) {
							...
							float srcWidth = bmdraw.getBitmap().getWidth();
							float srcHeight = bmdraw.getBitmap().getHeight();
							float dstWidth = (float)picdata.mWidth * mDrawScale;
							float dstHeight = (float)picdata.mHeight * mDrawScale;

							// 見開き対応モードで、画像が横長のとき
							// 1ページに画像の半分だけ表示する
							// 表示座標がマイナスのときは2画面目の画像を表示する
							switch (mPicScale) {
								case TextManager.BMPSCALE_DUAL_SCREEN:
								case TextManager.BMPSCALE_DUAL_SCREEN_NO_MARGIN:
									if (picdata.mWidth > picdata.mHeight) {
										Bitmap bm;
										if (tdd.mTextX >= 0) {
											drawX = (int)(x + mDrawWidth - ((float) tdd.mTextX + picdata.mWidth / 2) * mDrawScale);
											bm = Bitmap.createBitmap(bmdraw.getBitmap(), bmdraw.getBitmap().getWidth() / 2, 0, bmdraw.getBitmap().getWidth() / 2, bmdraw.getBitmap().getHeight(), null, true);
											bmdraw = new BitmapDrawable(bm);
											bmdraw.setBounds(0, 0, bm.getWidth(), bm.getHeight());
										} else {
											bm = Bitmap.createBitmap(bmdraw.getBitmap(), 0, 0, bmdraw.getBitmap().getWidth() / 2, bmdraw.getBitmap().getHeight(), null, true);
											bmdraw = new BitmapDrawable(bm);
											bmdraw.setBounds(0, 0, bm.getWidth(), bm.getHeight());
										}
									}
			...
			try {
				...
				if (mImageMgr != null) {
					if (mImageMgr.getFileType() != FileData.FILETYPE_DIR) {
						...
						mWorkStream = new WorkStream(mActivity, filename, mUser, mPass, mHandler);
						try {
							bm = BitmapFactory.decodeStream(mWorkStream, null, option);
						}
	...
	public void updateOverSize() {
		...
		if (mDispMode == DEF.DISPMODE_TX_HALF || mDispMode == DEF.DISPMODE_TX_DUAL) {
			if (mViewPoint == DEF.VIEWPT_CENTER) {
				mDrawLeft = (disp_x - view_x) / 2l;
				mDrawTop = (disp_y - view_y) / 2l;
			}
			else {
			...
			if (mViewPoint == DEF.VIEWPT_CENTER) {
				mDrawLeft = (disp_x - view_x) / 2l;
				// 右基準
				mDrawLeft = (mDispWidth - mDrawWidth) / 2l - page1_x;
			}
			else {
				boolean isLeft = false;
				if (mViewPoint == DEF.VIEWPT_LEFTTOP || mViewPoint == DEF.VIEWPT_LEFTBTM) {
	...
	public int TextScaling() {
		...
		if (mSclMode == DEF.SCALE_PINCH) {
			...
			if (mSclMode == DEF.SCALE_FIT_ALL) {
				// 全体を表示
                // Y方向よりもX方向の方が拡大率が小さいときにtrue逆ならfalse
                fWidth = disp_x * 1000 / src_cx < disp_y * 1000 / src_cy;
			}
			else {
				// 幅に合わせるならtrueそれ以外はfalse
				fWidth = mSclMode == DEF.SCALE_FIT_WIDTH;
			}
...
//		update();
	...
	private int getCurrentPage(int base) {
		...
		if (base == PAGEBASE_LEFT) {
			...
			// 中央
			pos = (int) (mDrawWidthSum - mDispWidth / 2l + mDrawLeft);
		}
		else {
			return -1;
		}
