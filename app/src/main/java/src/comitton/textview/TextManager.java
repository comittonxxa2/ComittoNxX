...
public class TextManager {
	...
	private static final int COMMENT_JISAGE = 1;
	private static final int COMMENT_JISAGE_RANGE = 2;
	private static final int COMMENT_PAGECONTROL = 3;
	private static final int COMMENT_STRCODE = 4;
	private static final int COMMENT_STRCODE_EXT = 5;
	private static final int COMMENT_PICTURE = 6;
	private static final int COMMENT_PICTURE_CHAR = 7;
	private static final int COMMENT_MIDASHI = 8;
	private static final int COMMENT_MIDASHI_ST = 9;
	private static final int COMMENT_MIDASHI_ED = 10;
	private static final int COMMENT_TEXTSTYLE = 11;
	private static final int COMMENT_TEXTSTYLE_ST = 12;
	private static final int COMMENT_TEXTSTYLE_ED = 13;
	private static final int COMMENT_JITUKI = 14;

	private final int PARAM_PAGEBREAK = 0;
	private final int PARAM_PAGECENTER = 1;

	public static final int TB_PICTURE = -1;
	public static final int TB_PAGECONTROL = -2;

	private static final int CHARTYPE_INIT = -1;
	private static final int CHARTYPE_KANJI = 0;
	private static final int CHARTYPE_ASCII = 1;
	private static final int CHARTYPE_KANA = 2;
	private static final int CHARTYPE_CTRL = 3;
	private static final int CHARTYPE_SYMBOL = 4;
	private static final int CHARTYPE_MARK = 5;
	private static final int CHARTYPE_PCHAR = 6;
	private static final int CHARTYPE_ASCII_NUM = 7;
	private static final int CHARTYPE_ASCII_MARK = 8;

	public static final int BMPSCALE_ORIGINAL = 0;
	public static final int BMPSCALE_ZOOM2X = 1;
	public static final int BMPSCALE_ZOOM3X = 2;
	public static final int BMPSCALE_ZOOM4X = 3;
	public static final int BMPSCALE_SCREEN = 4;
	public static final int BMPSCALE_SCREEN_NO_MARGIN = 5;
	public static final int BMPSCALE_DUAL_SCREEN = 6;
	public static final int BMPSCALE_DUAL_SCREEN_NO_MARGIN = 7;
...
//	private StyleManager mTextStyle;
	...
	public class EpubFile {
		...
		public EpubFile(String id, String href, String type) {
			this.id = id;
			this.href = href;
			this.type = type;
		}
	...
	public class MidashiData {
		...
		public static final int EPUB_TITLE = 1;
		public static final int EPUB_TOC = 2;
		public static final int FILE_TITLE = 3;
		public static final int HEADING_LARGE = 4;
		public static final int HEADING_MIDDLE = 5;
		public static final int HEADING_SMALL = 6;
	...
	public class MidashiManager {
		...
		public MidashiData addMidashiData(String text, int line, int type) {
...
//				}
	...
	public void LoadTextFile() {
		LoadTextFile(mTextFile, true);
	}

	public void LoadTextFile(String filename) {
		LoadTextFile(filename, true);
	}
...
//		Logcat.d(logLevel, MessageFormat.format("終了します.", new Object[]{mInputBuff.length}));
	...
	public FileListItem[] getEpubImageList() {
		...
		if (mEpubMode == EPUB_MODE_COVER) {
			if (mCover != null && !mCover.isEmpty()) {
				for (int i = 0; i < ImageList.length; ++i) {
					if (mCover.equals(ImageList[i].name)) {
				...
				if (!mImageList.isEmpty()) {
					...
					loop: for (int i = 0; i < mImageList.size(); ++i) {
						for (int j = 0; j < ImageList.length; ++j) {
							if (mImageList.get(i).equals(ImageList[j].name)) {
					...
					// 画像リストがなければファイルタイプが画像の最初のファイルを返す
					for (int i = 0; i < ImageList.length; ++i) {
						if (ImageList[i].type == FileData.FILETYPE_IMG) {
							list.add(ImageList[i]);
							break;
						}
					}
		...
		else if (mEpubMode == EPUB_MODE_ALL_IMAGE) {
			if (!mImageList.isEmpty()) {
				// 画像リストがあれば画像リストの順番に並べ替えて返す
		...
		return list.toArray(new FileListItem[0]);
	}
			...
			while (eventType != XmlPullParser.END_DOCUMENT) {
				try {
					...
					if (eventType == XmlPullParser.START_TAG) {
						...
						if (tag_name.equalsIgnoreCase("title")) {
						...
						else if (tag_name.equalsIgnoreCase("creator")) {
							tag_level_creator++;
						}
						else if (tag_name.equalsIgnoreCase("publisher")) {
							tag_level_publisher++;
						}
						else if (tag_name.equalsIgnoreCase("meta")) {
							String name = "";
							String content = "";
							for (int i = 0; i < atr_count; i++) {
								String atr_name = xmlPullParser.getAttributeName(i);
								if (atr_name.equalsIgnoreCase("name")) {
									name = xmlPullParser.getAttributeValue(i);
								}
								if (atr_name.equalsIgnoreCase("content")) {
									content = xmlPullParser.getAttributeValue(i);
								}
							}
							if (name.equalsIgnoreCase("primary-writing-mode")) {
								// テキストの行の方向（縦書き、横書き）
						...
						else if (tag_name.equalsIgnoreCase("item")) {
							if (tag_level_manifest > 0) {
								...
								for (int i = 0; i < atr_count; i++) {
									...
									if (atr_name.equalsIgnoreCase("href")) {
										href = DEF.relativePath(mActivity, filename, xmlPullParser.getAttributeValue(i));
									}
									if (atr_name.equalsIgnoreCase("media-type")) {
										type = xmlPullParser.getAttributeValue(i);
									}
									if (atr_name.equalsIgnoreCase("properties")) {
						...
						else if (tag_name.equalsIgnoreCase("spine")) {
							tag_level_spine++;
							String id = "";
							for (int i = 0; i < atr_count; i++) {
								String atr_name = xmlPullParser.getAttributeName(i);
						...
						else if (tag_name.equalsIgnoreCase("itemref")) {
							if (tag_level_spine > 0) {
								...
								for (int i = 0; i < atr_count; i++) {
									String atr_name = xmlPullParser.getAttributeName(i);
									if (atr_name.equalsIgnoreCase("idref")) {
										id = xmlPullParser.getAttributeValue(i);
									}
								}
								if (!id.isEmpty()) {
						...
						else if (tag_name.equalsIgnoreCase("guide")) {
							tag_level_guide++;
						}
						else if (tag_name.equalsIgnoreCase("reference")) {
							if (tag_level_guide > 0) {
								...
								for (int i = 0; i < atr_count; i++) {
									String atr_name = xmlPullParser.getAttributeName(i);
									if (atr_name.equalsIgnoreCase("title")) {
										id = xmlPullParser.getAttributeValue(i);
									}
									if (atr_name.equalsIgnoreCase("href")) {
										href = DEF.relativePath(mActivity, filename, xmlPullParser.getAttributeValue(i));
									}
									if (atr_name.equalsIgnoreCase("type")) {
										type = xmlPullParser.getAttributeValue(i);
									}
								}
								if (!href.isEmpty()) {
					...
					try {
						...
						if (eventType == XmlPullParser.START_TAG) {
							...
							if (tag_name.equalsIgnoreCase("navPoint")) {
								...
								tag_level_text++;
							} else if (tag_name.equalsIgnoreCase("content")) {
								for (int i = 0; i < atr_count; i++) {
									String atr_name = xmlPullParser.getAttributeName(i);
									if (atr_name.equalsIgnoreCase("src")) {
										href = DEF.relativePath(mActivity, mTocFile, xmlPullParser.getAttributeValue(i));
									}
								}
							}
				...
				try {
					...
					if (eventType == XmlPullParser.START_DOCUMENT) {
						...
						if (tag_name.equalsIgnoreCase("br")) {
							...
							inputSB.append("\n");
							text_cnt = 0;
						} else if (tag_name.equalsIgnoreCase("ruby")) {
							tag_level_ruby++;
							if (tag_level_ruby == 1) {
								inputSB.append("｜");
							}
						} else if (tag_name.equalsIgnoreCase("rt")) {
							tag_level_rt++;
							if (tag_level_rt == 1) {
								inputSB.append("《");
							}
						} else if (tag_name.equalsIgnoreCase("rp")) {
							tag_level_rp++;
						} else if (tag_name.equalsIgnoreCase("p")) {
							tag_level_p++;
							text_cnt = 0;
							isDiv = false;
						} else if (tag_name.equalsIgnoreCase("img") || tag_name.equalsIgnoreCase("image")) {
							...
							for (int i = 0; i < atr_count; i++) {
								String atr_name = xmlPullParser.getAttributeName(i);
								if (atr_name.equalsIgnoreCase("class")) {
									class_value = xmlPullParser.getAttributeValue(i);
								}
								if (atr_name.equalsIgnoreCase("src") || atr_name.equalsIgnoreCase("href")) {
									if (mFileType == FileData.FILETYPE_EPUB) {
										src_value = DEF.relativePath(mActivity, filename, xmlPullParser.getAttributeValue(i));
									} else {
										src_value = xmlPullParser.getAttributeValue(i);
									}
								}
								if (atr_name.equalsIgnoreCase("alt")) {
									alt_value = xmlPullParser.getAttributeValue(i);
								}
							}
							if (!src_value.equalsIgnoreCase("")) {
								if (class_value.equalsIgnoreCase("gaiji") || class_value.equalsIgnoreCase("gaiji-line") || class_value.equalsIgnoreCase("gaiji-wide")) {
									if (alt_value.equalsIgnoreCase("")) {
										inputSB.append("［＃外字（");
									} else {
									...
									if (mEpubMode == EPUB_MODE_ALL_IMAGE || mEpubMode == EPUB_MODE_FIRST_IMAGE) {
										// イメージビュワーなら挿絵を保存
										mImageList.add(src_value);
										if (mEpubMode == EPUB_MODE_FIRST_IMAGE) {
											// 最初の挿絵だけを保存して終了
											return title;
										}
									}
							...
							tag_level_body++;
						} else if (tag_name.equalsIgnoreCase("div")) {
							if (!isDiv && inputSB.length() != 0) {
								inputSB.append("\n");
								isDiv = true;
							}
						} else if (tag_name.equalsIgnoreCase("h1") || tag_name.equalsIgnoreCase("h2") || tag_name.equalsIgnoreCase("h3")) {
							// 見出しとする
							tag_level_h++;
							line_text = "";
							text_cnt = 0;
						} else if (tag_name.equalsIgnoreCase("ol")) {
							inputSB.append("\n");
						}
					} else if (eventType == XmlPullParser.END_TAG) {
						...
						if (tag_name.equalsIgnoreCase("ruby")) {
							if (tag_level_ruby > 0) {
								tag_level_ruby--;
							}
						} else if (tag_name.equalsIgnoreCase("rt")) {
							...
							if (!line_text.isEmpty()) {
								...
								if (tag_name.equalsIgnoreCase("h1")) {
									inputSB.append("大見出し］\n");
								}
								if (tag_name.equalsIgnoreCase("h2")) {
									inputSB.append("中見出し］\n");
								}
								if (tag_name.equalsIgnoreCase("h3")) {
									inputSB.append("小見出し］\n");
								}
							}
							text_cnt = 0;
						...
						} else if (tag_name.equalsIgnoreCase("li") || tag_name.equalsIgnoreCase("dt") || tag_name.equalsIgnoreCase("dd")) {
							inputSB.append("\n");
						}
					} else if (eventType == XmlPullParser.TEXT) {
						//Logcat.d(logLevel, "Text '" + xmlPullParser.getText() + "'");
	...
	public int mEpubMode = EPUB_MODE_ALL;
	public static int EPUB_MODE_ALL = 0;
	public static int EPUB_MODE_COVER = 1;
	public static int EPUB_MODE_FIRST_IMAGE = 2;
	public static int EPUB_MODE_ALL_IMAGE = 3;
	...
	ArrayList<EpubFile> mSpine = new ArrayList<EpubFile>();
	...
	public void formatTextFile(int width, int height, float headfont, float textfont, float rubifont, float space_w, float space_h, int margin_w, int margin_h, int pic_scale, String fontfile, int ascmode) {
		...
		if (mEpubMode == EPUB_MODE_ALL || mEpubMode == EPUB_MODE_ALL_IMAGE || mEpubMode == EPUB_MODE_COVER) {
			// EPUBコンテナファイルの解析
			...
			if (filename.toLowerCase().endsWith(".opf")) {
				readEpubRoot(filename, inputSB);
			}
		}

		if (mEpubMode == EPUB_MODE_ALL) {
			// 目次情報ファイルの解析
			if (mFileType == FileData.FILETYPE_EPUB) {
				readEpubTOC();
			}
		...
		if (mEpubMode == EPUB_MODE_ALL || mEpubMode == EPUB_MODE_ALL_IMAGE || mEpubMode == EPUB_MODE_FIRST_IMAGE) {
			if (mSpine.isEmpty()) {
				mSpine.add(new EpubFile("id1", filename, ""));
			}

			// 本文xhtmlの解析
			int progress = 0;
			DEF.sendMessage(mHandler, DEF.HMSG_HTML_PARSE, 0, mSpine.size(), null);
			for (int epubFileIndex = 0; epubFileIndex < mSpine.size(); epubFileIndex++) {
				++progress;
	...
	private LineManager layoutLines(ArrayList<LineData> linedata) {
		...
		for (int linecnt = 2; linecnt < linemax && linecnt < 500 && commed == 0; linecnt++) {
			...
			if (ld.textblock.length >= 1 && ld.textblock[0].tx_length >= 40) {
				...
				for (int i = 0; i < ld.textblock[0].tx_length; i++) {
					if (ld.textblock[0].tx_index + i >= 0 && ld.textblock[0].tx_index + i < mTextBuff.length) {
						// コメント行チェック
		...
		int prev_progress = 0;
		int progress = 0;
		DEF.sendMessage(mHandler, DEF.HMSG_TX_LAYOUT, 0, 0, null);
		...
		for (int linecnt = 0; linecnt < linemax; linecnt++) {
			...
			for (int tbidx = 0; tbidx < ld.textblock.length; tbidx++) {
				...
				while (true) {
					...
					while (tl > 0) {
						...
						if (0 < tl_adj) {
							// 文字あり
							if (!rr.ascii && tx_rest_cy - font_cy * tl_adj < font_cy) {
			...
			if (linemax > 0) {
				progress = linecnt * 50 / linemax;
			}
			if (progress != prev_progress) {
				prev_progress = progress;
				DEF.sendMessage(mHandler, DEF.HMSG_TX_LAYOUT, progress, 0, null);
			}
	...
	private TextDrawData[][] layoutPages(LineManager dm) {
		...
		int prev_progress = 0;
		int progress = 0;
		DEF.sendMessage(mHandler, DEF.HMSG_TX_LAYOUT, 50, 0, null);
		...
		while (true) {
			...
			if (textlist != null && textlist.size() == 1 && textlist.get(0).mTextPos == TB_PICTURE) {
				...
				if (pd != null) {
					...
					switch (mPicScale) {
						...
						case BMPSCALE_SCREEN_NO_MARGIN:
					...
					switch (mPicScale) {
						...
						case BMPSCALE_DUAL_SCREEN:
							pagedata = new ArrayList<TextDrawData>();
							if (pd.mWidth < pd.mHeight) {
								// ページに全体表示
								td.mTextX = (mWidth - pd.mWidth) / 2;
							} else {
								// 2画面表示
								td.mTextX = mTextRight + mTextWidth - (pd.mWidth / 2);
								...
								td.mTextX = mTextRight - (pd.mWidth / 2);
								td.mTextY = (mHeight - pd.mHeight) / 2; // 高さは画面中央へ
							}
							break;
						case BMPSCALE_DUAL_SCREEN_NO_MARGIN:
			...
			if (linemax > 0) {
				progress = linecnt * 50 / linemax + 50;
			}
			if (progress != prev_progress) {
				prev_progress = progress;
				DEF.sendMessage(mHandler, DEF.HMSG_TX_LAYOUT, progress, 0, null);
	...
	private CommentResult analyzeComment(String comment) {
		...
		}
		{
			...
			if (cmd != -1) {
				...
				//
				if (code == '小') {
					type = MidashiData.HEADING_SMALL;
				}
				else if (code == '中') {
					type = MidashiData.HEADING_MIDDLE;
				}
				else if (code == '大') {
					type = MidashiData.HEADING_LARGE;
				}
		...
		if (comment.startsWith("［＃「")) {
			...
			int type = 0; // 注記のサブタイプ
			int len = 0; //
			if (comment.endsWith("」はEPUBタイトル］")) {
				cmd = COMMENT_MIDASHI;
				type = MidashiData.EPUB_TITLE;
				len = size - 14;
			}
			if (comment.endsWith("」はEPUB目次］")) {
				cmd = COMMENT_MIDASHI;
				type = MidashiData.EPUB_TOC;
				len = size - 12;
			}
			if (comment.endsWith("」はファイルタイトル］")) {
				cmd = COMMENT_MIDASHI;
				type = MidashiData.FILE_TITLE;
				len = size - 14;
			}
			...
			else if (comment.endsWith("」は小見出し］")) {
				cmd = COMMENT_MIDASHI;
				type = MidashiData.HEADING_SMALL;
				len = size - 10;
			}
	...
	private void parseTextData(char[] inbuff, int inoff, StringBuffer textbuff, ArrayList<LineData> linedata) {
		...
		int prev_progress = 0;
		int progress = 0;
		DEF.sendMessage(mHandler, DEF.HMSG_TX_PARSE, 0, 0, null);
				...
				if (skipcnt > 0) {
					...
					switch (cr.code) {
						...
						case COMMENT_MIDASHI: {
							...
							String label = comment.substring(3, cr.param2 + 3);
							mMidashi.addMidashiData(label.replaceAll("[　 \t]+", "　"), linecnt, cr.param1);
							break;
						}
						...
						case COMMENT_MIDASHI_ED:
							if (midashiMode == 1 && midashiLabel.length() > 0) {
								...
								mMidashi.addMidashiData(label, midashiLine, cr.param1);
...
//					}
					...
					if (code != '\n') {
						...
						if (linecnt >= 2 && textclm == 0 && textcolumn2 > 0 && textcolumn2 < 40 && last_char2 != '。' && last_char2 != '）' && textcolumn3 == 0) {
							...
							if (ld != null) {
								...
								mMidashi.addMidashiData(sb.toString().replaceAll("^[　 \t]+", "").replaceAll("[　 \t]+", "　"), linecnt - 1, MidashiData.CAPTION);
							}
						}
						...
						// 進捗の表示
						if (size > 0) {
							progress = inpos * 100 / size;
						}
						if (progress != prev_progress) {
							prev_progress = progress;
							DEF.sendMessage(mHandler, DEF.HMSG_TX_PARSE, progress, 0, null);
						}
					}
				}
				inpos++; // 次の文字へ
	...
	/**
	 * 画像の拡大縮小後の幅と高さをmPicArray配列へ挿入する
	 * @param filename 画像ファイル名
	 * @param isCharacter 挿絵なら0、外字なら1
	...
	private int loadBitmap(String filename, boolean isCharacter) {
		...
		if (isCharacter) {
			// 文字の置き換え
			//picdata.mWidth = src_cx;
			//picdata.mHeight = src_cy;
			dst_cx = src_cx;
			dst_cy = src_cy;
			if (dst_cx > mTextSize || dst_cy > mTextSize) {
				scale_x = (float) mTextSize / (float) src_cx;
				scale_y = (float) mTextSize / (float) src_cy;
			}
			else {
			...
			switch (mPicScale) {
				...
				case BMPSCALE_DUAL_SCREEN:
					...
					scale_x = (float) mTextWidth * 2 / (float) src_cx;
				...
				case BMPSCALE_DUAL_SCREEN_NO_MARGIN:
					...
					scale_x = (float) mWidth * 2 / (float) src_cx;
					scale_y = (float) mHeight / (float) src_cy;
					break;
				case BMPSCALE_ZOOM2X:
					dst_cx = src_cx * 2;
					dst_cy = src_cy * 2;
